...
- **Post-migration audit (2026-07-03):** a codebase re-check against the new schema found **3 runtime bugs in untested surfaces** the mechanical `?`→`%s` port missed — all now fixed + regression-tested: (1) `market_brief._build_brief_payload` joined `daily_ohlc` on a non-existent `symbol` column and selected `o.close`/`sm.company_name`/`sm.sector` (rewritten to `stock_id`/`close_adj` + `stock_master`/`sector_master` joins); (2) DuckDB-only `ROUND(double, n)` in `market_brief` + `sectors.py` (Postgres needs `ROUND(CAST(... AS numeric), n)`); DuckDB alias-in-`ORDER BY ABS(alias)` (Postgres needs an outer subquery); (3) `ROUND(numeric)` returns `Decimal`, which broke `market_brief`'s own `json.dumps(payload)` (cast to float). Added `tests/api/test_sectors.py` + `tests/ai/test_market_brief.py`. Gap noted: sectors/news/strategies/alerts/watchlists lack API tests.
- **Full-stack run-check (2026-07-03):** brought up Docker (TimescaleDB+Mongo) + FastAPI (:8000) + Next.js (:3000), seeded 8 stocks × 260 sessions, and exercised every read endpoint. Two more bugs found + fixed + regression-tested: (4) `app/indicators/compute.py` — the indicator engine used DuckDB `?` placeholders and `conn.executemany()` (psycopg has neither), so `compute_all` never ran on Postgres (tests seeded indicators directly). Fixed → `%s` + cursor.executemany; `tests/indicators/test_compute_persist.py`. (5) **pre-existing (not migration):** `app/scanners/moving_average.py` used `10.0 * scale(...)` where `scale()` returns 0–100, so composites blew past the 0–100 contract (a neutral slope alone added 500; observed 540). Fixed → `0.1 * (...)` matching the rsi/volume scanners; `tests/scanners/test_ma_composite_range.py`. NOTE: the `pg` test fixture truncates the shared analytics tables, so demo data must be re-seeded after running the suite.
- **Status:** Accepted (implemented in Phase 3; post-migration SQL bugs fixed 2026-07-03)
...
| D-052 | 2026-07-01 | BONUS transaction encodes extra shares in `quantity` (not new total); SPLIT factor encodes in `price` field (zero monetary meaning for splits); FIFO reducer detects `type=BONUS`/`SPLIT` and handles specially — rationale: single Transaction schema covers all corp-action types without extra fields | Backend/Finance | `app/portfolio/positions.py` | Accepted |
| D-053 | 2026-07-01 | Alert dedup key: `{type}:{symbol}:{discriminator}:{as_of_date}` — one alert per real event per day; `alert_events` UNIQUE constraint on `(dedup_key, as_of_date)` ensures idempotent EOD re-runs; guardrail-suppressed alerts are logged with `guardrail_status="suppressed"`, never silently dropped | Backend/Compliance | `app/alerts/engine.py` | Accepted |

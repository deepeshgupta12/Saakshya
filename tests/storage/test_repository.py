...
def test_restatement_creates_new_version_preserving_history(pg) -> None:
    repo = Repository(pg)
...
def test_reingest_same_version_is_idempotent_update(pg) -> None:
    repo = Repository(pg)
...
def test_upsert_stock_is_idempotent_by_symbol(pg) -> None:
    repo = Repository(pg)
